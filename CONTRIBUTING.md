...
Gerfaut's code is licensed under the GNU Affero General Public License v3.0 (AGPL-3.0-only), with copyright kept centralized — this is what makes commercial licensing and signed store builds possible, which fund the project.
...
Gerfaut is watch-only. **No private key material ever enters the codebase**: no key generation, no seed handling, no signing. Pull requests introducing any of it will be closed.
